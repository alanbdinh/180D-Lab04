...
prevXAcc = 0
prevZAcc = 0 #place these outside to track acc across loop iterations
...
while True:
    ...
    # simple classifier-- read normalized X and mag Z for fwd push, upwd lift motions
    XchangeThres = abs(0.05*prevXAcc) # larger change threshold reduces sensitivity to static drift/noise
    ZchangeThres = abs(0.1*prevZAcc) # must change by percentage of old value
    posXAcc = (magXcomp - prevXAcc > XchangeThres) 
    posZAcc = (MAGz - prevZAcc < -1*ZchangeThres) # negative Z value is upward
    prevXAcc = magXcomp
    prevZAcc = MAGz
    ...
    if posXAcc:
      print('norm X accel = %5.2f ' % magXcomp + 'forward push\n') 
    if posZAcc:
      print('Z accel = %5.2f ' % MAGz + 'upward lift\n')
    # print(outputString)
